...
var BasicRequest = inherit(
    ...
    },
    {
        ...
        request : function (options) {
            ...
            if (!self._request) {
                if (options) {
                    RequestOptions.mapOptions.call(self, options);
                }
            ...
            if (isRedirect(response) && self.options.followRedirects) {
                try {
                    ...
                    if (self.options.redirect_count > 10) {
                        ...
                        self.emit('error', null, 'Failed to follow redirect over 10 redirects');
                    }
 ...
 * Or you could even pass an object with the options set e.g.
 * new Service('myURL', {
        'options' {
            auth    : function (request) {
                //My Special Authentication
            }
        }
   });
 * See the tests for details
 */
var Service = function (baseURL, ServiceRequest) {
    var actions = {
            newRequest: function (path, options) {
    ...
    //The ServiceRequest must be a rational form of a Request
    //Since the Request object does the best job of parsing it's own
    //options, I create an instance and let it do the parsing
    if (typeof ServiceRequest === 'function') {
        if (Request.prototype.isPrototypeOf(ServiceRequest.prototype)) {
            //The ServiceRequest class is a Request, so we just use it.
            ServiceRequest = new ServiceRequest(baseURL);
        } else {
            //TODO throw type error
            ServiceRequest = new Request(baseURL);
        }
    } else if (typeof ServiceRequest === 'object') {
        //I am assuming that you have given me an object that you want to use
        //as your options
        ServiceRequest = new Request(baseURL, ServiceRequest);
    }
    
    //Now I have an instance of a Request
    if (ServiceRequest instanceof Request) {            
        //Create a base class that inherits these options from this request
        ServiceRequest = inherit(
 ...
 */
var service = function (constructor, baseURL, methods, ServiceRequest) {
    mixin(constructor.prototype, new Service(baseURL, ServiceRequest));
 ...
 * 
 */
var customRequest = function (methods) {
